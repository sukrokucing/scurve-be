...
fn get_required_text(row: &SqliteRow, col: &str) -> Result<String, AppError> {
    ...
    Err(AppError::internal(format!("missing {}: decode failed", col)))
}
...
fn get_required_i32(row: &SqliteRow, col: &str) -> Result<i32, AppError> {
    ...
    if let Ok(v) = row.try_get::<i64, _>(col) {
        return i32::try_from(v).map_err(|_| AppError::internal(format!("invalid {} out of range: {}", col, v)));
    }
    if let Ok(v) = row.try_get::<f64, _>(col) {
        if v.fract() == 0.0 {
            let as_i64 = v as i64;
            return i32::try_from(as_i64).map_err(|_| AppError::internal(format!("invalid {} out of range: {}", col, v)));
        }
        return Err(AppError::internal(format!("invalid {} not integer: {}", col, v)));
    }
    if let Ok(v) = row.try_get::<String, _>(col) {
        return v.trim().parse::<i32>().map_err(|e| AppError::internal(format!("invalid {}: {}", col, e)));
    }
