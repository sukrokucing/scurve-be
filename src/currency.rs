/// ISO 4217 active currency codes (as of 2026). Subset covers all currencies
/// with current issuance; historical/withdrawn codes are intentionally omitted.
/// Source: ISO 4217 current code list.
const ISO_4217: &[&str] = &[
...
/// Validate an ISO 4217 currency code. Accepts mixed case; returns the
/// uppercase canonical code if valid.
pub fn normalize(code: &str) -> Option<String> {
    ...
    if ISO_4217.binary_search(&upper.as_str()).is_ok() {
        Some(upper)
    } else {
        None
    }
